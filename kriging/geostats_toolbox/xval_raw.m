function [Zpred]=xval_raw(obs,beta,maxpoints,maxdist,Gmodel,Kmodel,nugget)
...
%       ordinary)
% nugget = nugget effect; (optional, defaults to 0)
% 
% Outputs:
...
for i=1:length(pred)                        % For every row in pred (every sampled point)
    ...
    [ZpredTemp, VpredTemp] = krig(obsTemp,predTemp,beta,maxpoints,maxdist,Gmodel,Kmodel,nugget); % Kriging
